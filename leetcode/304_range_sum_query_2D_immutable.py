...
* int sumRegion(int row1, int col1, int row2, int col2) Returns the sum of the elements of matrix
  inside the rectangle defined by its upper left corner (row1, col1) and lower right corner
  (row2, col2).
    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        for i in range(1, len(matrix) + 1):
            for j in range(1, len(matrix[0]) + 1):
                ...
                self.pre_sum[i][j] = (
                        self.pre_sum[i-1][j]
                        + self.pre_sum[i][j-1] + matrix[i-1][j-1] - self.pre_sum[i-1][j-1])

    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        return (
            self.pre_sum[row2+1][col2+1] - self.pre_sum[row1][col2+1]
            - self.pre_sum[row2+1][col1] + self.pre_sum[row1][col1]
        )
