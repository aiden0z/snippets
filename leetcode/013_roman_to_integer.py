...
class Solution:
    ...
    def romanToInt(self, s):
        ...
        while i < length:
            value = roman[s[i]]
            if value in (1, 10, 100) and i + 1 < length and roman[s[i + 1]] > value:
                value = roman[s[i + 1]] - value
                i += 2
            else:
                i += 1
            count += value
...
if __name__ == '__main__':
    cases = [(3, 'III'), (4, 'IV'), (9, 'IX'), (58, 'LVIII'), (1994, 'MCMXCIV'),
             (1776, 'MDCCLXXVI'), (2014, 'MMXIV'), (1954, 'MCMLIV')]
    s = Solution()
    sb = Solution()

    for case in cases:
        print(s.romanToInt(case[1]), case[1])
        assert s.romanToInt(case[1]) == case[0]
        assert sb.romanToInt(case[1]) == case[0]
