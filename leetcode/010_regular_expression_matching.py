    ...
    def isMatch(self, s, p):
        ...
        for i in range(s_len + 1):
            for j in range(1, p_len + 1):
                if p[j - 1] == '*':
                    m[i][j] = m[i][j - 2] or (i > 0 and (s[i - 1] == p[j - 2] or p[j - 2] == '.')
                                              and m[i - 1][j])
                else:
                    m[i][j] = i > 0 and m[i - 1][j - 1] and (s[i - 1] == p[j - 1]
                                                             or p[j - 1] == '.')
        return m[s_len][p_len]


if __name__ == '__main__':
    cases = [('aa', 'a', False), ('aa', 'aa', True), ('aaa', 'aa', False), ('aa', 'a*', True),
             ('aa', '.*', True), ('ab', '.*', True), ('aab', 'c*a*b', True)]
